...
func handlePodDNSMetricsAll(w http.ResponseWriter, r *http.Request) {
	aggregated := buildPodDNSResponse()
	...
	errors := make([]peerError, 0)
	peers := comm.ListPeerIPs(true)
	...
	for _, ip := range peers {
		...
		url := fmt.Sprintf("http://%s:8080/api/dns/pods", ip)
		resp, err := client.Get(url)
		if err != nil {
			errors = append(errors, peerError{Peer: ip, Error: err.Error()})
			continue
		}
		func() {
			defer resp.Body.Close()
			if resp.StatusCode >= 400 {
				errors = append(errors, peerError{Peer: ip, Error: fmt.Sprintf("status %d", resp.StatusCode)})
				return
			}
			...
			if err := json.NewDecoder(resp.Body).Decode(&payload); err != nil {
				errors = append(errors, peerError{Peer: ip, Error: fmt.Sprintf("decode: %v", err)})
				return
			}

			for podKey, metrics := range payload.Pods {
				if metrics == nil {
					continue
				}
				if _, exists := metrics["node_ip"]; !exists {
					metrics["node_ip"] = ip
				}
				if _, exists := aggregated[podKey]; !exists {
					aggregated[podKey] = metrics
				} else {
					// Preserve existing entry; only note conflict if data differs
					aggregated[podKey]["conflict"] = true
				}
			}
		}()
	}
	...
	json.NewEncoder(w).Encode(map[string]interface{}{
		...
		"peers_responded":   len(peers) - len(errors),
		"peer_errors":       errors,
